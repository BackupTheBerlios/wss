...
class object_c: public message_c 
{
	...
	object_c ();
	virtual ~object_c ();
	static object_c* dyncreate ( unsigned long type );
	...
	virtual void coocked_to_local ( object_c* o, matrix_c < FT > & lbase, matrix_c <FT >& lvA, int& flag);	memory_map_c visual_memories;
	object_c* parent;
};
